...
class TaskHandler:
    ...
    def assigned(self, task):
        ...
        if task == 1:
            return sum([1 for robot in globals.ROBOTS if robot.task == task and robot.has_to_work()]) + sum([1 for robot in globals.ROBOTS if robot.task == 0 and robot.has_to_work()]), sum([1 for robot in globals.ROBOTS if robot.task == task and not robot.has_to_work()]) + sum([1 for robot in globals.ROBOTS if robot.task == 0 and not robot.has_to_work()])
        else:
